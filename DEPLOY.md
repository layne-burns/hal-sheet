...
**The party's dossiers are already there.** Sol, Karlie, Gill, Dinos and Qee arrive with the sheet, with their faces, their standing and their stories. They are inserted **once** and then belong to you: edit Gill's card and the edit is yours forever, and a later app update that changes the shipped text changes nothing on a sheet that already has it. Delete one and it stays deleted — the sheet remembers, so an update won't resurrect them. A dossier added in a future update simply appears.
...
**Scrolling inside a window** used to snap back to the top on the iPad. Three causes, all fixed. The root one: the whole-app zoom used to sit on `<body>`, which put the modal overlay — a `position: fixed` box — inside a zoomed subtree, and WebKit lays such a box out in the zoomed coordinate space while hit-testing and scrolling it against the unzoomed one. The zoom now lives on the app container instead, so the overlay is fixed in plain viewport coordinates. On top of that, the scroller is the overlay rather than a max-height box nested inside it, the page behind is properly locked while a window is open (rather than relying on `overscroll-behavior`, which Safari only learned in 16), and the scroll position survives a re-render instead of being thrown away every time you tapped something.
...
**Companions** (Combat → Followers) are everything that came along without being conjured: a bought horse, a mule, a dog. Give one a name, a face, a role and an owner. The role decides how the app treats it — **Mount** and **War beast** fight and roll their own initiative; **Riding animal**, **Pack animal** and **Companion** never appear in the turn order at all, because an order with the baggage in it is an order you stop reading. A non-combatant rides on its owner's portrait instead, as a small badge in the bottom-right corner. Give Gil the mule and it appears on Gil.

HP is optional and usually blank. A summoned steed has a real HP bar because the spell gives it real numbers; a mule doesn't, for the same reason the party roster tracks a status and not a number. Type a Max HP and the bar appears.

**Mount and dismount.** A **Mount** button sits in the combat strip next to the movement it spends, and on each mount's own panel. It opens a window rather than just happening, because getting on costs half your Speed and doing that by mis-tap is hard to argue back. Pick the rider (defaults to Hal) and the mount; it shows the mount's speed, its role, and what it will cost. **If Hal is riding and a fight is running, half his Speed is deducted** per the 2024 rules. If an ally is riding, nothing is deducted — their movement is theirs to spend and this sheet has never tracked it.

While mounted, the mount **gives up its own turn** and is drawn chained to its rider in the turn order, two faces on one chip. Dismount and it takes a slot again, immediately after the rider. A summoned **familiar** always rolls its own initiative — that's the whole point of it — while the **steed** never does, because it shares your count.

**Ties are settled by hand.** Two combatants on the same roll get ↑↓ arrows in the initiative sheet; nobody else does, since nudging an untied row would be undone by the sort. The order you leave them in is the order they act in.
...
**Writing something down.** While a session is running, the app nudges you if it's been a while since your last note — and on a feast day it says which. The whole strip is one tap into a window with room to work: pick what it's about (**Met someone**, **Learned something**, **Deal or debt**, **A fight**, **Found something**, **Somewhere new**, or **Just a note**) and the boxes change to the questions that kind actually asks. Meeting somebody asks who, where, and what they wanted, which is three things you already know and would never have typed into a one-line box.

Whatever you fill in composes down to a single readable line, shown before you save it so there are no surprises. Meeting someone also offers to **add them to People** — you've just typed a name, a place and a want, which is a People record already. The window never opens itself: a window that appears because a timer went off is a window you learn to dismiss.
